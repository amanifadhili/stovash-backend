import { Injectable, OnModuleInit } from '@nestjs/common';
import { EventBus } from '@electronic-shop/framework-event';
import { saleCreatedConsumer } from './consumers/sale-created.consumer.js';

@Injectable()
export class EventConsumerService implements OnModuleInit {
  private eventBus: EventBus;

  constructor() {
    this.eventBus = new EventBus({
      url: process.env.RABBITMQ_URL || 'amqp://localhost:5672',
      exchangeName: 'electronic-shop-events',
      queuePrefix: 'customer-service',
    });
  }

  async onModuleInit() {
    try {
      // Connect to RabbitMQ
      await this.eventBus.connect();

      // Create consumer
      const saleConsumer = this.eventBus.createConsumer('customer-sales', 'sale.created');

      // Register handler
      this.eventBus.registerHandler('customer-sales', 'SaleCreated', saleCreatedConsumer);

      // Start consuming
      await this.eventBus.startAllConsumers();

      console.log('Event consumers registered and started for Customer Service');
    } catch (error) {
      console.error('Failed to initialize event consumers:', error);
    }
  }
}
